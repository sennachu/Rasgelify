...
const PendingReservations = () => {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        ...
        const userPendingReservations = response.data.filter(
          reservation => reservation.boat.owner._id === userId && reservation.status === 'pending'
        );
  ...
  return (
    <div className='container mx-auto p-6'>
      <h2 className='text-3xl font-semibold mb-4'>Bekleyen Rezervasyonlar</h2>
      {reservations.length === 0 ? (
        <div className='text-center text-lg text-gray-500'>Bekleyen rezervasyon yok.</div>
      ) : (
